...
export function OverridesList({ perPositionTargets, onReset }) {
  ...
  return (
    <div className="max-w-[420px]">
      <h2 className="mb-1 text-[14px] font-bold">Per-stock hard caps</h2>
        ...
        <div className="divide-y divide-border rounded-DEFAULT border border-border bg-surface">
          {overrides.map(([ticker, dollars]) => (
            <div key={ticker} className="flex items-center justify-between px-4 py-3">
              <Link to={`/stocks/${ticker}`} className="text-[13.5px] font-semibold text-accent hover:underline">
                {ticker}
              </Link>
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="rounded-lg border border-border-strong bg-surface px-3 py-1.5 text-[12.5px] font-semibold hover:bg-surface-sunken disabled:opacity-50"
                >
                  Reset to default
                </button>
              </div>
