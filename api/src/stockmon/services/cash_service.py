...
def _load_cash_event_rows(db: Session, user_id: int) -> list[CashEventRow]:
    return (
        db.query(CashEventRow)
        .filter(CashEventRow.user_id == user_id)
        .order_by(CashEventRow.event_date, CashEventRow.id)
        .all()
    )


def load_all_trade_flow_events(db: Session, user_id: int) -> list[CashFlowEvent]:
    """Every trade, every ticker, for this user -- the cash side of buys/sells."""
    rows = (
        db.query(TradeRow)
...
def load_all_cash_event_flow_events(db: Session, user_id: int) -> list[CashFlowEvent]:
    return [cash_row_to_flow_event(row) for row in _load_cash_event_rows(db, user_id)]
...
def list_cash_events(db: Session, user_id: int) -> tuple[list[CashEventEntry], Decimal]:
    """This user's events newest-first, plus their current cashAvailable."""
    rows = _load_cash_event_rows(db, user_id)
    cash_available = compute_cash_balance(
        [cash_row_to_flow_event(row) for row in rows] + load_all_trade_flow_events(db, user_id)
    )
    entries = [
...
def record_cash_event(
    db: Session, user_id: int, type_: Literal["deposit", "withdraw"], amount: Decimal, event_date: date
) -> tuple[CashEventRow, Decimal]:
    """Validates fields, builds the candidate full event list (existing
    trades + existing cash events + this new one) for this user, validates
    via validate_cash_sequence BEFORE inserting -- atomic, never partial."""
    ...
    row = CashEventRow(user_id=user_id, type=type_, amount_usd=amount, event_date=event_date)
    db.add(row)
    db.commit()
    db.refresh(row)
...
def delete_cash_event(db: Session, user_id: int, event_id: int) -> Decimal:
    """Builds the candidate list with this event removed, validates, then
    deletes. Raises CashNotFoundError if the id doesn't exist OR belongs to
    another user (identical externally-visible behavior either way)."""
    row = db.get(CashEventRow, event_id)
    if row is None or row.user_id != user_id:
        raise CashNotFoundError(event_id)

    remaining_rows = [r for r in _load_cash_event_rows(db, user_id) if r.id != event_id]
    candidate = load_all_trade_flow_events(db, user_id) + [cash_row_to_flow_event(r) for r in remaining_rows]
